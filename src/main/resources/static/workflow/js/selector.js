...
;(function($, window, document, undefined) {
	var dialog = function(element, opt) {
		...
		var defaultOpt = {//默认配置
			...
			commit : function(data) {//回填--回调函数
				...
				self.element.data("code", data);//把唯一code也回填到元素data-code上。
		...
		selectStaff : function(target) {
			...
			if (select_check === "single") {// ******单选
				...
				if (item.find("span").hasClass("icon-check")) { // 取消选中**删除
					...
					var dept = item.find("span").data("dept");
					if(dept != null && dept != "")
						dept = "("+dept+")";
					this.addSelectedItem(select_id, select_type, staff_text+(dept || ""));
				}
			}
		},
		/**
	...
	$.fn.staffSelector = function(option) {
		...
		var defaultOption = {data:$(self).data("code")};
