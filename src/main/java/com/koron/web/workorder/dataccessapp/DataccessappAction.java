...
@Api(tags = "it服务-数据接入申请")
@RequestMapping("/workorder/DataccessappAction")
@RestController
public class DataccessappAction implements WorkflowFinishInterface {

    Logger log = Logger.getLogger(DataccessappAction.class);

    @ApiOperation("---提交数据接入申请")
    @PostMapping(value = "/addDataccessapp.htm")
    public MessageBean<?> addDataccessapp(@RequestBody DataccessappBean bean) {
        ...
        try {
            ...
            bean = ADOConnection.runTask(new DataccessappService(), "addDataccessapp", DataccessappBean.class,bean);
            ...
            log.error("提交数据接入申请异常", e);
            info.setCode(Constant.MESSAGE_INT_UNKNOW_ERROR);
            info.setDescription("提交数据接入申请异常");
            return info;
        }
        return info;
    }
    ...
    @PostMapping(value = "/updateDataccessapp.htm")
    public MessageBean<?> updateDataccessapp(@RequestBody DataccessappBean bean, HttpServletRequest req) {
        ...
        try {
            ...
            bean = ADOConnection.runTask(new DataccessappService(), "updateDataccessapp", DataccessappBean.class,bean);
            ...
            log.error("数据接入申请-审核前保存表单-异常", e);
            info.setCode(Constant.MESSAGE_INT_UNKNOW_ERROR);
            info.setDescription("数据接入申请-审核前保存表单-异常");
            return info;
        }
        return info;
    }

    @ApiOperation("---删除数据接入申请")
    ...
    public MessageBean<?> delete(@RequestParam("id") String id, HttpServletRequest req) {
        ...
        try {
            ...
            info = ADOConnection.runTask(new DataccessappService(), "deleteDataccessapp", MessageBean.class,id);
        } catch (Exception e) {
            e.printStackTrace();
            log.error("数据接入申请-删除异常", e);
            info.setCode(Constant.MESSAGE_INT_UNKNOW_ERROR);
            info.setDescription("数据接入申请-删除异常");
            return info;
        }
        return info;
    }

    @ApiOperation("---查询数据接入申请列表")
    @PostMapping(value = "/queryList.htm")
    public MessageBean<?> list(@RequestBody DataccessappQueryBean queryBean) {
        ...
        try {
            ...
            List<DataccessappBean> list = ADOConnection.runTask( new DataccessappService(), "queryList", List.class,queryBean);

            info.setData(new PageInfo<>(list));
        } catch (Exception e) {
            log.error("数据接入申请-列表异常", e);
            info.setCode(Constant.MESSAGE_INT_UNKNOW_ERROR);
            info.setDescription("数据接入申请-列表异常");
            return info;
        }
        return info;
    }
    ...
    public MessageBean<?> detail(@RequestParam("id") String id) {
        ...
        @SuppressWarnings("rawtypes")
        MessageBean<DataccessappBean> info = MessageBean.create(Constant.MESSAGE_INT_SUCCESS, "success", DataccessappBean.class);
        try {

            DataccessappBean bean = ADOConnection.runTask( new DataccessappService(), "detail", DataccessappBean.class,id);

            info.setData(bean);
        } catch (Exception e) {
            log.error("数据接入申请-查询详情", e);
            info.setCode(Constant.MESSAGE_INT_UNKNOW_ERROR);
            info.setDescription("数据接入申请-查询详情");
            return info;
        }
        return info;
    }
    ...
    public Integer workflowStart(Object dataBean, StaffunBean user) {
        DataccessappBean bean =(DataccessappBean) dataBean;
        bean.setProcInstName("数据接入申请");//工作流平台 名称
        WorkflowBusiness workflowBusiness = new WorkflowBusiness(bean.getId(), user, WorkflowUtil.WORKFLOW_TYPE_DATAACCAPP);
    ...
    public Integer workflowFinishDo(StaffunBean user, WorkflowBean bean) {
        DataccessappBean DataccessappBean = new DataccessappBean();
        DataccessappBean.setId(bean.getProcInstId());
        //设置启动
        DataccessappBean.setStatus(1);//流程生效状态
        MessageBean<?> runTask = ADOConnection.runTask(new DataccessappService(), "finishDo", MessageBean.class, DataccessappBean);
        if(runTask != null){
