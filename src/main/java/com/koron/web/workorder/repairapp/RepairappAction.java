...
@Api(tags = "it服务-报障报修申请")
@RequestMapping("/workorder/RepairappAction")
@RestController
public class RepairappAction implements WorkflowFinishInterface {

    Logger log = Logger.getLogger(RepairappAction.class);

    @ApiOperation("---提交报障报修申请")
    @PostMapping(value = "/addRepairapp.htm")
    public MessageBean<?> addRepairapp(@RequestBody RepairappBean bean) {
        ...
        try {
            ...
            bean = ADOConnection.runTask(new RepairappService(), "addRepairapp", RepairappBean.class,bean);
            ...
            log.error("提交报障报修申请异常", e);
            info.setCode(Constant.MESSAGE_INT_UNKNOW_ERROR);
            info.setDescription("提交报障报修申请异常");
            return info;
        }
        return info;
    }
    ...
    @PostMapping(value = "/updateRepairapp.htm")
    public MessageBean<?> updateRepairapp(@RequestBody RepairappBean bean, HttpServletRequest req) {
        ...
        try {
            ...
            bean = ADOConnection.runTask(new RepairappService(), "updateRepairapp", RepairappBean.class,bean);
            ...
            log.error("报障报修申请-审核前保存表单-异常", e);
            info.setCode(Constant.MESSAGE_INT_UNKNOW_ERROR);
            info.setDescription("报障报修申请-审核前保存表单-异常");
            return info;
        }
        return info;
    }

    @ApiOperation("---删除报障报修申请")
    ...
    public MessageBean<?> delete(@RequestParam("id") String id, HttpServletRequest req) {
        ...
        try {
            ...
            info = ADOConnection.runTask(new RepairappService(), "deleteRepairapp", MessageBean.class,id);
        } catch (Exception e) {
            e.printStackTrace();
            log.error("报障报修申请-删除异常", e);
            info.setCode(Constant.MESSAGE_INT_UNKNOW_ERROR);
            info.setDescription("报障报修申请-删除异常");
            return info;
        }
        return info;
    }

    @ApiOperation("---查询报障报修申请列表")
    @PostMapping(value = "/queryList.htm")
    public MessageBean<?> list(@RequestBody RepairappQueryBean queryBean) {
        ...
        try {
            ...
            List<RepairappBean> list = ADOConnection.runTask( new RepairappService(), "queryList", List.class,queryBean);

            info.setData(new PageInfo<>(list));
        } catch (Exception e) {
            log.error("报障报修申请-列表异常", e);
            info.setCode(Constant.MESSAGE_INT_UNKNOW_ERROR);
            info.setDescription("报障报修申请-列表异常");
            return info;
        }
        return info;
    }
    ...
    public MessageBean<?> detail(@RequestParam("id") String id) {
        ...
        @SuppressWarnings("rawtypes")
        MessageBean<RepairappBean> info = MessageBean.create(Constant.MESSAGE_INT_SUCCESS, "success", RepairappBean.class);
        try {

            RepairappBean bean = ADOConnection.runTask( new RepairappService(), "detail", RepairappBean.class,id);

            info.setData(bean);
        } catch (Exception e) {
            log.error("报障报修申请-查询详情", e);
            info.setCode(Constant.MESSAGE_INT_UNKNOW_ERROR);
            info.setDescription("报障报修申请-查询详情");
            return info;
        }
        return info;
    }
    ...
    public Integer workflowStart(Object dataBean, StaffunBean user) {
        RepairappBean bean =(RepairappBean) dataBean;
        bean.setProcInstName("报障报修申请");//工作流平台 名称
        WorkflowBusiness workflowBusiness = new WorkflowBusiness(bean.getId(), user, WorkflowUtil.WORKFLOW_TYPE_REPAIRAPP);
...
//                map.put("assignercode", bean.getAssignercode());
    ...
    public Integer workflowFinishDo(StaffunBean user, WorkflowBean bean) {
        RepairappBean RepairappBean = new RepairappBean();
        RepairappBean.setId(bean.getProcInstId());
        //设置启动
        RepairappBean.setStatus(1);//流程生效状态
        MessageBean<?> runTask = ADOConnection.runTask(new RepairappService(), "finishDo", MessageBean.class, RepairappBean);
        if(runTask != null){
