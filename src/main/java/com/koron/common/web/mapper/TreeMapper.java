...
public interface TreeMapper {
     ...
     * @return
     */
	@Select("select COALESCE(max(key),0) from (select row_number() over() as key,a.seq >> (62-parentmask-mask)  & (1 << mask )-1 as value from tbltree a where a.type = #{type} and seq > #{lower} and seq < #{upper} and (seq & ((1 << #{mask}) -1)) = 0) as t where t.key = t.value")
